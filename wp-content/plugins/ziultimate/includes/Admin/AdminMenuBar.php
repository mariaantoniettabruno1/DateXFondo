...
class AdminMenuBar {
	...
	public function zu_add_link_to_admin_bar( $admin_bar ) {
		...
		if( Requirements::passed_pro_plugin_requirements() ) {
			...
			/*$theme_builders = Plugin::instance()->templates->get_templates_by_type('theme_builder');
			if( $theme_builders ) 
			{
				foreach( $theme_builders as $key => $theme_builder ) {
					$admin_bar->add_node([
						'parent' => Whitelabel::get_id() . '-theme-builder',
						'id'     => Whitelabel::get_id() . '-tb-' . $key,
						'title'  => esc_attr( $theme_builder->post_title ),
						'href'   => esc_url( admin_url( 'admin.php?page=zionbuilder-theme-builder&post_id=' . $theme_builder->ID . '&action=zion_builder_active' ) ),
					]);
				}
			}*/
		}
