...
class AdditionalPageOptions {
	...
	public function __construct() {
		...
		add_filter( 'zionbuilder/cache/page_js', [ $this, 'add_custom_javascript_to_page' ], 10, 2 );
	...
	public function add_custom_javascript_to_page($js, $post_id) {
		// Add post custom javascript
		$custom_js = get_post_meta( $post_id, self::PAGE_CUSTOM_JS_META_KEY, true );

		if ( ! empty( $custom_js ) ) {
			$js .= sprintf( 'try {%s} catch (error) {console.error(error)};', $custom_js );
		}
