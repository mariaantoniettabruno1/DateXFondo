/*global jQuery, GravityActionsLicense, document, ajaxurl */

( function ( $ ) {
    'use strict';

    var GravityActionLicenseSettings = {
        message: '',
        ...
        init: function () {
            GravityActionLicenseSettings.message_fadeout();
            GravityActionLicenseSettings.add_status_container();

            $( document )
              .on( 'ready keyup', GravityActionLicenseSettings.license_field, GravityActionLicenseSettings.key_change )
              .on( 'click', '.gv-edd-action', GravityActionLicenseSettings.clicked )
              .on( 'gv-edd-failed gv-edd-invalid', GravityActionLicenseSettings.failed )
              .on( 'gv-edd-valid', GravityActionLicenseSettings.valid )
              .on( 'gv-edd-deactivated', GravityActionLicenseSettings.deactivated )
              .on( 'gv-edd-inactive gv-edd-other', GravityActionLicenseSettings.other );
        },
        ...
        add_status_container: function () {
            $( GravityActionsLicense.license_box ).insertBefore( GravityActionLicenseSettings.license_field );
        },
        ...
        clicked: function ( e ) {
            ...
            $that.not( GravityActionLicenseSettings.check_button ).addClass( 'button-disabled' );

            $( '#gform-settings,#gform-settings .button' ).css( 'cursor', 'wait' );

            GravityActionLicenseSettings.set_pending_message( $that.attr( 'data-pending_text' ) );

            GravityActionLicenseSettings.post_data( theData );
        },

        post_data: function ( data ) {
            $.post( ajaxurl, {
                'action': 'gravityactions_license',
                'data': data
            }, function ( response ) {

                response = $.parseJSON( response );

                GravityActionLicenseSettings.message = response.message;
                ...
                GravityActionLicenseSettings.update_status( response.message );
        ...
        valid: function () {
            GravityActionLicenseSettings.activate_button
              .fadeOut( 'medium', function () {
                  GravityActionLicenseSettings.activate_button.removeClass( 'button-disabled' );
                  GravityActionLicenseSettings.deactivate_button.fadeIn().css( 'display', 'inline-block' );
              } );
        },

        failed: function () {
            GravityActionLicenseSettings.deactivate_button.removeClass( 'button-disabled' );
            GravityActionLicenseSettings.activate_button.removeClass( 'button-disabled' );
        },

        deactivated: function () {
            GravityActionLicenseSettings.deactivate_button
              .css( 'min-width', function () {
                  return $( this ).width();
              } )
              .fadeOut( 'medium', function () {
                  GravityActionLicenseSettings.deactivate_button.removeClass( 'button-disabled' );
                  GravityActionLicenseSettings.activate_button.fadeIn( function () {
        ...
        other: function () {
            GravityActionLicenseSettings.deactivate_button.fadeOut( 'medium', function () {
                GravityActionLicenseSettings.activate_button
    ...
    GravityActionLicenseSettings.init();
