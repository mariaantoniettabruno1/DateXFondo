...
abstract class GravityView_Extension {
	...
	public function settings() {
		...
		if( !class_exists( 'GV\Inline_Edit\EDD_SL_Plugin_Updater' ) ) {
		...
		new GV\Inline_Edit\EDD_SL_Plugin_Updater(
	...
	protected function is_extension_supported() {
		...
		if( !class_exists( 'GravityView_Plugin' ) ) {
			...
			$message = sprintf( __('Could not activate the %s Extension; GravityView is not active.', 'gravityview-inline-edit'), esc_html( $this->_title ) );

		} else if( false === version_compare(GravityView_Plugin::version, $this->_min_gravityview_version , ">=") ) {

			$message = sprintf( __('The %s Extension requires GravityView Version %s or newer.', 'gravityview-inline-edit' ), esc_html( $this->_title ), '<tt>'.$this->_min_gravityview_version.'</tt>' );

		} else if( isset( $this->_min_php_version ) && false === version_compare( phpversion(), $this->_min_php_version , ">=") ) {

			$message = sprintf( __('The %s Extension requires PHP Version %s or newer. Please ask your host to upgrade your server\'s PHP.', 'gravityview-inline-edit' ), esc_html( $this->_title ), '<tt>'.$this->_min_php_version.'</tt>' );
			...
			$message = sprintf( __( 'The %s Extension is not compatible with this version of GravityView. Please update the Extension to the latest version.', 'gravityview-inline-edit' ), esc_html( $this->_title ) );
