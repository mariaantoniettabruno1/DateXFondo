...
namespace GravityKit\GravityExport\GravityKit\QueryFilters;
...
class QueryFilters {
	...
	private function get_user_role_choices() {
		...
		$editable_roles['current_user'] = [
			'name' => esc_html__( 'Any Role of Current User', 'gk-query-filters' ),
		];
	...
	private function get_field_filters() {
		...
		$field_filters[] = [
			...
			'text'      => esc_html__( 'Created By User Role', 'gk-query-filters' ),
		...
		if ( ! in_array( 'date_updated', $field_keys, true ) ) {
			$field_filters[] = [
				'key'       => 'date_updated',
				'text'      => esc_html__( 'Date Updated', 'gk-query-filters' ),
				'operators' => array( 'is', '>', '<' ),
		...
		if ( $category_fields = \GFAPI::get_fields_by_type( $this->form, array( 'post_category' ) ) ) {
		...
		foreach ( $field_filters as &$filter ) {
			...
			if ( 'created_by' === $filter['key'] ) {
				...
				$filter['text'] = esc_attr__( 'Created By', 'gk-query-filters' );

				$current_user_filters = [
					[
						'text'  => __( 'Currently Logged-in User (Disabled for Administrators)', 'gk-query-filters' ),
						'value' => 'created_by_or_admin',
					],
					[
						'text'  => __( 'Currently Logged-in User', 'gk-query-filters' ),
						'value' => 'created_by',
					],
				];
	...
	private function get_translations() {
		...
		$translations = apply_filters( 'gk/query-filters/translations', [
			'internet_explorer_notice' => esc_html__( 'Internet Explorer is not supported. Please upgrade to another browser.', 'gk-query-filters' ),
			'fields_not_available'     => esc_html__( 'Form fields are not available. Please try refreshing the page.', 'gk-query-filters' ),
			...
			'remove_field'             => esc_html__( 'Remove Field', 'gk-query-filters' ),
			'available_choices'        => esc_html__( 'Return to Field Choices', 'gk-query-filters' ),
			'available_choices_label'  => esc_html__( 'Return to the list of choices defined by the field.', 'gk-query-filters' ),
			'custom_is_operator_input' => esc_html__( 'Custom Choice', 'gk-query-filters' ),
			'untitled'                 => esc_html__( 'Untitled', 'gk-query-filters' ),
			'field_not_available'      => esc_html__( 'Form field ID #%d is no longer available. Please remove this condition.', 'gk-query-filters' ),
		] );
	...
	public function enqueue_scripts( array $meta = array() ) {
		...
		wp_localize_script( $handle, 'gkQueryFilters', [
